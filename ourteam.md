...
interface CircularGalleryProps {
  // Updated interface to match your requirements: Image, Name, LinkedIn
  items: { image: string; name: string; linkedin: string }[];
  textColor?: string;
  scrollSpeed?: number;
}

const CircularGallery = ({ items, textColor = '#ffffff', scrollSpeed = 2 }: CircularGalleryProps) => {
                            ...
                            <div className="relative w-full h-full group">
                                ...
                                <div className="w-full h-full rounded-xl overflow-hidden border border-[#d4af37]/30 bg-[#0f0f12] shadow-[0_0_20px_rgba(0,0,0,0.5)] transition-all duration-300 group-hover:shadow-[0_0_30px_rgba(212,175,55,0.3)] flex flex-col">
                                    ...
                                    <div className="h-[25%] flex flex-col items-center justify-center bg-gradient-to-b from-[#15151a] to-[#0a0a0c] border-t border-[#d4af37]/20 relative z-10 px-4 gap-2">
                                        ...
                                        {/* LinkedIn Button - Dynamically uses item.linkedin */}
                                        <a 
                                            href={item.linkedin}
                                            target="_blank"
                                ...
                                <div className="absolute top-full left-0 w-full h-1/2 bg-gradient-to-b from-[#ffffff]/10 to-transparent opacity-30 transform scale-y-[-1] pointer-events-none mask-image-linear-gradient" style={{ maskImage: 'linear-gradient(to bottom, rgba(0,0,0,1), transparent)' }}></div>
                            </div>
                        </div>
                    );
                })}
...
const teamGalleryItems = [
  ...
  { 
    image: "https://api.dicebear.com/9.x/notionists/svg?seed=Neha&backgroundColor=b88a44", 
    name: "Neha Gupta", 
    linkedin: "https://www.linkedin.com/in/nehagupta" 
  },
  { 
    image: "https://api.dicebear.com/9.x/notionists/svg?seed=Rahul&backgroundColor=b88a44", 
    name: "Rahul Verma", 
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#d4cfc3] p-4 md:p-8 selection:bg-[#3d2b1f] selection:text-[#d4af37] flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="max-w-6xl w-full relative z-10 flex flex-col items-center justify-center h-full" ref={containerRef} onMouseMove={handleMouseMove}>
        
        {/* === OUR TEAM (CIRCULAR GALLERY) === */}
        <div className="space-y-12 w-full">
             ...
             <div className="w-full h-[600px] relative border border-[#d4af37]/20 rounded-lg overflow-hidden bg-[#050505]/50 backdrop-blur-md shadow-[0_0_50px_rgba(0,0,0,0.8)]">
                <CircularGallery 
